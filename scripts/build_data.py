    ...
    for el in soup.find_all(["h4", "table"]):
        ...
        for tr in el.find_all("tr"):
            ...
            try:
                ...
                continue
            penalties.append({
                "driver_raw": current, "date": f"{dm.group(3)}-{int(dm.group(2)):02d}-{int(dm.group(1)):02d}",
                "event": tds[1], "session": tds[2] if len(tds) > 3 else "",
                "description": tds[3] if len(tds) > 4 else "", "points": pts,
            })
    penalties_ok = soup.find("h4") is not None
    if not penalties_ok:
        warnings.append("Penalty page layout changed (no driver headings found); using previous penalty data.")
except Exception as e:  # noqa: BLE001
    warnings.append(f"Penalty points source unavailable: {e}")

if not penalties_ok and OUT.exists():
    try:
        prev = json.load(open(OUT))
        penalties = prev.get("penalties", [])
        penalty_asof = prev.get("penalty_asof")
        for p in penalties:
            p["stale"] = True
...
for p in penalties:
    hit = [k for k in known if k in norm(p["driver_raw"])]
    p["driver"] = hit[0] if hit else norm(p["driver_raw"])
pen_by_driver = {}
for p in penalties:
